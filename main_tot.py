...
def exp_string(i, args):
    ...
    tail = '-'.join(
        f'{key}={value}'
        for key, value in vars(args).items()
        if key not in ['backbone', 'mo_method', 'mode', 'device', 'every', 'metric']
    ).replace('.', '$')
...
def train(args, exp_id, val_best):
    ...
    if args.backbone == 'BPRMF':
        ...
        backbone = LightGCNModel(user_size, item_size, args, dataset['train_matrix'])
    elif args.backbone == 'NGCF':
        backbone = NGCFModel(user_size, item_size, args, dataset['train_matrix'])
    elif args.backbone == 'MixRec':
        backbone = MixRecModel(user_size, item_size, args, dataset['train_matrix'])
    else:
        print("Backbone not supported")
        return -1

    if args.mo_method == 'ADA2FAIR':
        model = Ada2FairModel(
            backbone=backbone,
            train_matrix=dataset['train_matrix'],
            train_user_list=train_user_list,
            provider_ids=dataset['provider_ids'],
            args=args
        ).to(args.device)
    ...
    if args.mo_method not in ['None', 'ADA2FAIR']:
    ...
    try:
        ...
        for iter in range(args.n_epochs):
            ...
            if args.mo_method == 'ADA2FAIR':
                ...
                for _ in range(args.weight_epochs):
                    for _ in range(num_batches):
                        user, pos, neg = sampler.next_batch()
                        neg = np.squeeze(neg)

                        user_id = torch.from_numpy(user).long().to(args.device)
            ...
            for _ in tqdm(range(num_batches), desc='Batch Progress Bar'):
                ...
                if 'r' in args.mode:
                    if args.mo_method == 'ADA2FAIR':
                        loss['1'] = model.weighted_recommendation_loss(user_id, pos_id, neg_id)
                    else:
                        loss['1'] = model(user_id, pos_id, neg_id)
                else:
                    loss['1'] = torch.tensor(0.0, device=args.device)
                ...
                # Ada2Fair
                    ...
                    if 'p' in args.mode or 'u' in args.mode:
                        loss_p, loss_u = model.weight_loss(user_id)
                        loss['2'] = loss_u if 'u' in args.mode else torch.tensor(0.0, device=args.device)
                        loss['3'] = loss_p if 'p' in args.mode else torch.tensor(0.0, device=args.device)
                    else:
                        loss['2'] = torch.tensor(0.0, device=args.device)
                ...
                elif args.mo_method not in ['ADA2FAIR']:
                    scale = {'1': args.scale1, '2': 1.0 - args.scale1, '3': 1.0 - args.scale1, '4': 1.0 - args.scale1}

                if args.mo_method == 'ADA2FAIR':
                    batch_loss = loss['1']
                    history_losses['loss_1'].append((loss['1'].item(), loss['1'].item()))
                    if 'u' in args.mode:
                        history_losses['loss_2'].append((loss['2'].item(), loss['2'].item()))
                    if 'p' in args.mode:
                        history_losses['loss_3'].append((loss['3'].item(), loss['3'].item()))
            ...
            if args.mo_method == 'ADA2FAIR' and model.provider_fairness_weight is not None:
                p = model.provider_fairness_weight.detach().cpu().numpy()
                ...
                print(f'Head avg: {head.mean():.4f}')
                print(f'Tail avg: {tail.mean():.4f}')
            ...
            if (iter + 1) % args.every == 0 and evaluation == True:
                    ...
                    if val_metric > val_best:
                        ...
                        if args.backbone == 'BPRMF':
                            ...
                            if model.node_dropout > 0:
                                sampled_adj = model.sparse_dropout(
                                    model.adj,
                                    model.node_dropout,
                                    model.adj.nnz()
                                )
                            ...
                            user_emb = gu.detach().cpu().numpy()
                            item_emb = gi[:-1].detach().cpu().numpy()  # remove padding item
                ...
                if val_metric > local_val_best:
                    local_val_best = val_metric
...
if __name__ == '__main__':
    ...
    dataset, index_F, index_M, genre_mask, popular_dict, vec_pop, long_tail, short_head, train_aplt, train_user_tail_list = preprocessing(
        settings)
    num_items = dataset['train_matrix'].shape[1]
    provider_ids = np.zeros(num_items, dtype=np.int64)
