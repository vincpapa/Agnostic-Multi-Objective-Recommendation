...
def shannon_entropy_at_k(pred_list, k, num_items=None, normalize=True, eps=1e-12):
    """
    Shannon Entropy over the item exposure distribution induced by the
    top-k recommendation lists. This mirrors the Elliot SEntropy metric:
    p(i) is the fraction of recommendation slots occupied by item i.

    If normalize=True, the value is divided by the maximum attainable
    entropy log2(min(num_items, num_users * k)), yielding a value in [0, 1].
    """
    ...
    probs = counts / float(num_users * k)
    ...
    if num_items is None:
        support = min(int(topk.max()) + 1, num_users * k)
    else:
        support = min(int(num_items), num_users * k)
    if support <= 1:
        return 0.0
    return entropy / math.log2(support)
...
def compute_metrics(test_set, pred_list, metric):
    ...
    if metric == 'ndcg':
        ...
        # Normalized Shannon Entropy over top-k item exposure.
        # item_size is defined globally after preprocessing in __main__.
        return shannon_entropy_at_k(pred_list, k, num_items=item_size, normalize=True)
    '''
...
def normalize_loss(data, eps=1e-8):
    sigmoid = Sigmoid()
    mean = torch.mean(data)
    # print(mean)
    std_dev = torch.std(data, unbiased=False)
    # print(std_dev)
    z_scores = (data - mean) / (std_dev + eps)
...
def normalize_loss_wo_sigmoid(data, eps=1e-8):
    # sigmoid = Sigmoid()
    mean = torch.mean(data)
    # print(mean)
    std_dev = torch.std(data, unbiased=False)
...
def compute_differentiable_shannon_entropy(ranks_prov, args, eps=1e-12):
    """
    Differentiable approximation of normalized Shannon Entropy over the
    top-k item exposure distribution.

    The hard metric computes p(i) as the fraction of recommendation slots
    occupied by item i. Here the hard top-k indicator is replaced by the
    AMORe soft cutoff mask C = eta(R; k). We return one value per user so
    that the standard AMORe normalization can be computed batch-wise.
    """
    k = int(getattr(args, 'atk_ent', getattr(args, 'atk_div', getattr(args, 'atk_pro', 20))))
    c = (torch.tanh(-ranks_prov + k) + 1.0) / 2.0  # [B, I]

    # Normalize each user's soft recommendation mass. This mirrors the
    # 1/k per-user weighting used in the hard SEntropy implementation.
    user_mass = c / (c.sum(dim=1, keepdim=True) + eps)  # [B, I]

    # Soft item exposure distribution over the current mini-batch.
    p_item = user_mass.sum(dim=0) / (user_mass.size(0) + eps)  # [I]
    item_self_information = -torch.log2(p_item + eps)  # [I]

    # Per-user contribution to the exposure entropy.
    entropy_user = (user_mass * item_self_information.unsqueeze(0)).sum(dim=1)  # [B]

    # Normalization to [0, 1]. With a hard top-k list, at most B*k distinct
    # items can appear in a batch, and never more than the catalog size.
    max_support = min(ranks_prov.size(1), max(2, ranks_prov.size(0) * k))
    denom = torch.log2(torch.tensor(float(max_support), device=ranks_prov.device))
    entropy_user = entropy_user / (denom + eps)

    return torch.clamp(entropy_user, min=0.0, max=1.0)


def amore_metric_loss(metric_values, args):
    """Apply the same AMORe deviation loss used by nDCG and APLT."""
    raw = torch.square(1.0 - metric_values)
    if args.mo_method in ['ADAAMORE', 'AMORE_MGDA', 'AMORE_EPO', 'AMORE_SCALE']:
        return normalize_loss(raw).sum()
    elif args.mo_method in ['AMORE_ABL_WOS']:
        return normalize_loss_wo_sigmoid(raw).sum()
    elif args.mo_method in ['AMORE_ABL_WOZ']:
        return normalize_loss_wo_zeta(raw).sum()
    else:
        return raw.sum()
...
def train(args, exp_id, val_best):
    ...
    if args.mo_method in ['AMORE_EPO']:
        ...
        preference = np.array([args.scale1 if t == '1' else 1 - args.scale1 for t in tasks], dtype=float)
        ...
        for iter in range(args.n_epochs):
            ...
            for _ in tqdm(range(num_batches), desc='Batch Progress Bar'):
                ...
                if args.mo_method in ['AMORE_MGDA', 'AMORE_EPO', 'AMORE_SCALE', 'AMORE_ABL', 'AMORE_ABL_WOS', 'AMORE_ABL_WOZ']:
                    ...
                    if 'e' in args.mode:
                        # Optional masking for entropy: the evaluation protocol removes
                        # training interactions from the candidate set. For an exposure
                        # diversity objective such as Shannon Entropy, aligning the
                        # surrogate with this candidate set is important. This requires
                        # a second rank approximation only for the entropy objective.
                        if getattr(args, 'entropy_mask_train', True):
                            seen_mask_entropy = torch.as_tensor(
                                train_matrix[unique_u.detach().cpu().numpy()].toarray(),
                                dtype=torch.bool,
                                device=scores_all.device
                            )
                            scores_entropy = scores_all.masked_fill(seen_mask_entropy, -1e8)
                            ranks_entropy = ranker(scores_entropy)
                        else:
                            ranks_entropy = ranks_prov

                        sentropy = compute_differentiable_shannon_entropy(ranks_entropy, args)
                        loss['5'] = amore_metric_loss(sentropy, args)
                    else:
                        loss['5'] = torch.tensor(0.0, device=args.device)

                    if 's' in args.mode:
                        # Legacy mode: merge all non-backbone AMORe terms into a
                        # single auxiliary objective. This now also supports entropy.
                        aux_terms = [v for k, v in loss.items() if k != '1']
                        if len(aux_terms) > 0:
                            loss['4'] = sum(aux_terms) / len(unique_u)
                        else:
                            loss['4'] = torch.tensor(0.0, device=args.device)
                        for _k in list(loss.keys()):
                            if _k not in ['1', '4']:
                                loss[_k] = torch.tensor(0.0, device=args.device)
                    else:
                        for _t in tasks:
                            if _t != '1':
                                loss[_t] = loss[_t] / len(unique_u)
